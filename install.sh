...
#     ~/.local/bin, seeds ~/.claude-relay/config.toml from config.example.toml, then runs
#     `claude-relay init` (adopts ~/.claude into a seat per adopt_default) and verify.sh.
#
...
#     ~/.local/share/claude-relay and symlinks. Then runs `claude-relay init`.
#
...
ensure_dirs_and_config() {  # $1 = optional documented example config to copy (local mode)
  mkdir -p "${STATE_DIR}" "${STATE_DIR}/logs"
  if [ ! -f "${STATE_DIR}/config.toml" ] && [ -n "${1:-}" ] && [ -f "${1}" ]; then
    cp "${1}" "${STATE_DIR}/config.toml"
    echo "Seeded ${STATE_DIR}/config.toml from the documented example."
  fi
  chmod 600 "${STATE_DIR}/config.toml" 2>/dev/null || true
}

# Seed config if still missing (embedded template) AND adopt ~/.claude into a named seat, per
# [defaults].adopt_default. Idempotent. Uses the just-installed executable in BIN_DIR (covers
# uv/pipx/git/symlink, all of which land there) so it works even if BIN_DIR isn't on PATH yet.
run_init() {
  local cr=""
  if [ -x "${BIN_DIR}/claude-relay" ]; then
    cr="${BIN_DIR}/claude-relay"
...
if [ -n "${SCRIPT_DIR}" ] && [ -x "${SCRIPT_DIR}/bin/claude-relay" ] && [ -d "${SCRIPT_DIR}/relay" ]; then
  ...
  ensure_dirs_and_config "${REPO_DIR}/config.example.toml"
  run_init
  path_note
  echo
...
path_note
echo
if [ -x "${BIN_DIR}/claude-relay" ] || command -v claude-relay >/dev/null 2>&1; then
  echo "Smoke test — discovered seats:"
  "${BIN_DIR}/claude-relay" login-check 2>/dev/null || claude-relay login-check || true
fi
echo
